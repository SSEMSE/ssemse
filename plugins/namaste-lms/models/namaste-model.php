...
class NamasteLMS {
   static function install($update = false) {
   	...
   	update_option( 'namaste_version', "1.36");
   	if(!$update) self::init();
   ...
   static function menu() {
	...
	static function init() {
		...
		if($version != '1.36') self::install(true);
	...
	static function options() {
		...
		if(!empty($_POST['namaste_payment_options']) and check_admin_referer('save_payment_options', 'nonce_payment_options')) {
			...
			update_option('namaste_use_pdt', @$_POST['use_pdt']);
			update_option('namaste_pdt_token', $_POST['pdt_token']);
